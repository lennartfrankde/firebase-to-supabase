...
if (args.length < 1) {
  console.log(
    "Usage: node json2supabase.js <path_to_json_file> [<primary_key_strategy>] [<primary_key_name>]"
  );
  console.log(
    "  path_to_json_file: full local path and filename of .json input file"
  );
  console.log("  primary_key_strategy (optional):");
  console.log("    none (no primary key is added");
  console.log(
    "    serial (id SERIAL PRIMARY KEY) (autoincrementing 2-byte integer)"
  );
  console.log(
    "    smallserial (id SMALLSERIAL PRIMARY KEY) (autoincrementing 4-byte integer)"
  );
  console.log(
    "    bigserial (id BIGSERIAL PRIMARY KEY) (autoincrementing 8-byte integer)"
  );
  console.log(
    "    uuid (id UUID PRIMARY KEY DEFAULT uuid_generate_v4()) (randomly generated uuid)"
  );
  console.log(
    "    firestore_id (id TEXT PRIMARY KEY) (uses existing firestore_id random text as key)"
  );
  console.log(
              ...
              client.query(
                ...
                function (err, res) {
                  ...
                  if (err) {
                    ...
                    console.log(
                      "Fields found in existing table:",
                      res.rows.length
                    );
                    ...
                    if (res.rows.length > 0) {
                      var _loop_1 = function (attr) {
                        ...
                        var dataType =
                          (_a = res.rows.find(function (row) {
                            return row.column_name === attr;
                          })) === null || _a === void 0
                            ? void 0
                            : _a.data_type;
                        if (!dataType) {
                      ...
                      if (primary_key_strategy !== "none") {
                        columnDefinitions.push(
                          createPrimaryKey(primary_key_strategy)
                        );
                      }

                      for (const attr in fields) {
                        columnDefinitions.push(`"${attr}" ${fields[attr]}`);
                      }

                      const sql_1 = `create table "${tableName}" (${columnDefinitions.join(
                        ", "
                      )})`;
                      client.query(sql_1, function (err, res) {
                        if (err) {
                          if (
                            err.toString().endsWith("specified more than once")
                          ) {
                            console.log(
                              err.toString() +
                                ": try specifying a different <primary_key_name>"
                            );
                            quit();
                          } else {
      ...
      return [
        ...
        new Promise(function (resolve, reject) {
          ...
          jsonStream.on("data", function (_a) {
            ...
            recordCount++;
            if (recordCount === 1) {
              console.log("Sample record fields:", Object.keys(value));
            }
            for (var attr in value) {
              ...
              if (
                ...
                console.log(
                  "multiple field types found for field " +
                    attr +
                    ": " +
                    fields[attr] +
                    ", " +
          ...
          jsonStream.on("end", function () {
            console.log(
              "Field analysis complete. Records analyzed:",
              recordCount
            );
              ...
              return __generator(this, function (_b) {
                switch (_b.label) {
                  case 0:
                    ...
                    batchCount++;
                    console.log(
                      `Processing batch ${batchCount} (${totalRows} rows processed so far)`
                    );
                    return [
                      4 /*yield*/,
            ...
            return __awaiter(_this, void 0, void 0, function () {
              ...
              return __generator(this, function (_a) {
                switch (_a.label) {
                  case 0:
                    console.log(
                      `Importing final batch of ${insertRows.length} rows...`
                    );
                    return [
                      4 /*yield*/,
                      runSQL(makeInsertStatement(fields, insertRows)),
                  ...
                  case 1:
                    result = _a.sent();
                    console.log(
                      `Import complete. Total rows processed: ${totalRows}`
                    );
                    resolve("DONE");
                    return [2 /*return*/];
                }
        ...
        new Promise(function (resolve, reject) {
          ...
          client.query(sql, function (err, res) {
            if (err) {
              ...
              console.log(
                `SQL query completed successfully. Rows affected: ${res.rowCount}`
              );
              resolve(res);
            }
          });
...
function createPrimaryKey(primary_key_strategy) {
  switch (primary_key_strategy) {
    ...
    case "uuid":
      return (
        '"' + primary_key_name + '" UUID PRIMARY KEY DEFAULT uuid_generate_v4()'
      );
    case "firestore_id":
