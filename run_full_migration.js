...
async function runFullMigration() {
  try {
    ...
    if (!fs.existsSync(authDir)) {
      throw new Error(
        "Auth directory not found. Please ensure you are in the correct project directory."
      );
    }
    ...
    // Check for firebase-service.json
    const firestoreServiceFile = path.join(
      firestoreDir,
    ...
    if (checkFileExists("id_mappings.json")) {
      log(
        `📋 ID mappings created: ${getFileSize("id_mappings.json")}`,
        "green"
      );
    } else {
      throw new Error(
        "id_mappings.json was not created. Import may have failed."
      );
    }
    ...
    log(
      "\n✨ Your Firebase data has been successfully migrated to PocketBase!",
      "bright"
    );
  } catch (error) {
...
process.on("SIGINT", () => {
  ...
  log(
    "You can resume by running individual steps from the migration guide.",
    "cyan"
  );
  process.exit(0);
});
...
log("====================================", "cyan");
log(
  "This will perform a complete migration of your Firebase data to PocketBase."
);
log(
  "Make sure your PocketBase server is running and you have proper credentials configured.\n"
);
